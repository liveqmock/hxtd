...
function convOwner(){
		...
		if(ckObj.length > 0){
            ...
            var dict = ${dict};
            //生成树菜单
            jsUtil.easyTree.init(dict, childFun);
            //首次加载数据
            ...
            <tr class="{$T.row$index%2==1?'':'bg_c_blue'}">
                ...
                <td>
                    <c:choose>
                        ...
                        <c:otherwise><a href="${ctx}/customer/customer/toViewPage.do?id={$T.row.id}" class="toviewpage">{$T.row.name}</a></c:otherwise>
                    </c:choose>
                </td>
