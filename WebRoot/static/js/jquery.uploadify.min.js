...
(function(c) {
				...
				if (e[0]) {
					if (e[0] == "*") {
						...
						for (var m = 0; m < e.length; m++) {
							i.cancelUpload(e[m]);
							c("#" + e[m]).find(".data").removeClass("data").html(" - 取消上传");
	...
	var a = {
		onDialogOpen: function() {
			...
			//this.queueData.errorMsg = "文件格式限制，无法加入上传队列！";
		...
		onDialogClose: function(d, f, g) {
			...
			if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					showMsg({successFlag:false,message:this.queueData.errorMsg});
		...
		onSelect: function(h) {
			...
			for (var g in this.queueData.files) {
				...
				if (f.uploaded != true && f.name == h.name) {
					...
					showMsg({successFlag:false,message:"队列中存在文件:"+str});
					setTimeout(_hide(),RcmsAjax.global.showTime);
					this.cancelUpload(h.id);
					this.queueData.filesCancelled++;
					return false;
			...
			if (c.inArray("onSelectError", e.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
					//if (e.queueSizeLimit > f) {
						//this.queueData.errorMsg += "\n上传文件队列超过最大限制 (" + f + ").";
					//} else {
						//this.queueData.errorMsg += "\n每次最多上传文件数 (" + e.queueSizeLimit + ").";
						this.queueData.errorMsg += "每次最多上传文件数 (" + e.queueSizeLimit + ").";
					//}
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					//this.queueData.errorMsg += '\n文件 "' + d.name + '" 大小超过限制 (' + e.fileSizeLimit + ").";
					this.queueData.errorMsg += '文件 "' + d.name + '" 大小超过限制 (' + e.fileSizeLimit + ").";
					break;
				case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
					//this.queueData.errorMsg += '\n文件 "' + d.name + '" 为空.';
					this.queueData.errorMsg += '文件 "' + d.name + '" 为空.';
					break;
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					//this.queueData.errorMsg += '\n文件 "' + d.name + '" 类型错误 (' + e.fileTypeDesc + ").";
					this.queueData.errorMsg += '文件 "' + d.name + '" 类型错误 (' + e.fileTypeDesc + ").";
					break;
				}
			}
