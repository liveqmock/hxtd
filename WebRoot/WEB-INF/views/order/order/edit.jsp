		...
		var $obj = $(".bor_e28d1f", window.frames["dialogIframe"].document);
			if($obj.length > 0){
				...
				if(productFlag){
					...
					$("#txt_" + action).val($obj.find("td:eq(0)").text());
					// 起售金额
					var nameAndValue = $obj.find("td:eq(1)").text();
					nameAndValue = nameAndValue.substring(nameAndValue.indexOf(':')+1);
					$("#productSellMoney").val(nameAndValue);
					$("#purchaseMoney").val(nameAndValue);
					// 利率
					nameAndValue = $obj.find("td:eq(2)").text();
					$("#earningRate").val(nameAndValue.substring(nameAndValue.indexOf(':')+1).replace("%",""));
					// 赎回公式
					nameAndValue = $obj.find("td:eq(3)").text();
					$("#redeemFormula").val(nameAndValue.substring(nameAndValue.indexOf(':')+1));
					productFlag=false;
				}
			}
	});
...
<div>
	...
	<div class="ml35 mr35 mt20 block cb cb">
		...
		<b class="b3"></b>
		<b class="b4"></b>
		<div class="ie_head"><h1 class="f14 fbnone mt10 ml10 fl">订单详情信息</h1><ul class="fr id_table1 mt10 ml10"></ul></div>
	</div>
	...
	<div class="ml35 mr35 bg_c_blue cb">
		...
		<table class="cb id_table3 w95b bg_c_white margin0 mt10">
			<tr>
				...
				<td align="left">
					<input type="text" id="txt_owner" name="name_owner" value="${order.owner.realName}" 
			...
			<tr>
				...
				<td align="left">
					<input type="text" id="txt_product" name="name_product" value="${order.product.name}" 
						class="text_input3 cp required" onclick="searchData('product');" readonly/>
				...
				<td align="left">
					<input type="text" id="txt_customer" name="name_customer" value="${order.customer.name}" 
						class="text_input3 cp required" onclick="searchData('customer');" readonly/>
			...
			<tr>
				...
				<td align="left">
					<input type="text" name="purchaseMoney" id="purchaseMoney"
				...
				<td align="left"><input type="text" name="purchaseMoney" value="${order.purchaseMoney}" class="text_input3 number required"/></td>
				<td align="right"><span class="w_red">*&nbsp;</span>收益率（%）：</td>
				<td align="left"><input type="text" name="earningRate" id="earningRate" value="${order.earningRate}" 
					class="text_input3 number required" readonly/></td>
			...
			<tr>
				...
				<td align="left"><input type="text" name="redeemFormula" id="redeemFormula" value="${order.redeemFormula}" 
					class="text_input3 required" readonly/></td>
			</tr>
			<tr>
		...
		<table class="cb id_table4 w95b bg_c_white margin0 mt10">
			<tr>
				...
				<td align="left" width="85%"><textarea name="remark" class="remarks_input1">${order.remark}</textarea></td>
			</tr>
		</table>
		<div class="h40"></div>
	</div>
