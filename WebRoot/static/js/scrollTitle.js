...
// 根据滚动条切换
//=====================
RcmsFloatTitle.toggle = function(obj){
	if($('#recordDiv').length<0){
		return false;
	}
	//var titlePos = $('#recordDiv')[0].offsetTop;
	//老的方式判断出现问题，使用jquery提供的方法
	var titlePos = $('#recordDiv').offset().top;
	...
	if(topNow > titlePos){
		...
		if(!flag){
			RcmsFloatTitle.floatDiv.css('top',0);
			RcmsFloatTitle.floatDiv.css('position','fixed');
		}else{
			RcmsFloatTitle.floatDiv.css('top',topNow);
			RcmsFloatTitle.floatDiv.css('position','absolute');
		}
		RcmsFloatTitle.floatDiv.show();
	}else{
		RcmsFloatTitle.floatDiv.hide();
	}
}