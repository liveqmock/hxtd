...
public class ExcelParse4Contact extends ExcelParse<ContactDTO> {
	...
	public ContactDTO checkDataBySheetList(int rowNumOfSheet, List<String> sheetList) {
		...
		if ( Tools.isEmpty(contactName.trim()) ) {
			msg += "<br />第"+rowNumOfSheet+"条数据,联系人姓名不能为空!";
			logger.warn(msg);
			//向无效数据集合中添加一条数据
		...
		if ( !Tools.isEmpty(contactOwner.trim()) ) {
			...
			if(owner==null){
				msg += "<br />第"+rowNumOfSheet+"条数据,联系人所有者不存在!";
				logger.warn(msg);
				//向无效数据集合中添加一条数据
				ImportMessage.invalidFormatRowNumMap.put(rowNumOfSheet, msg);
				return null;
	...
	public void getDuplicateDate(Map<Integer, ContactDTO> entityMap,List<String> paramList) {
		...
		for(Integer rowNum1:keySet1){
			...
			String mobile1 = contact1.getMobile().trim();
			String email1 = contact1.getEmail().trim();
			...
			for(Integer rowNum2:keySet2){
				 ...
				 */
				String uniqueString = "";
				String mobile2 = contact2.getMobile().trim();
				String email2 = contact2.getEmail().trim();
