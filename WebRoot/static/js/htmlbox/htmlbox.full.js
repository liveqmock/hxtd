(function($) {
	$.fn.htmlbox = function(options) {
		...
		var styles = (typeof document.htmlbox_styles === 'function') ? document.htmlbox_styles() : [
			['No Styles', '', '']
		];
		var syntax = (typeof document.htmlbox_syntax === 'function') ? document.htmlbox_syntax() : [
			['No Syntax', '', '']
		];
		...
		var d = {
			...
			idir: jsUtil.getRootPath()+"/static/js/htmlbox/images/",
			icons: "default",
			about: true,
			skin: "default",
			output: "xhtml",
		...
		d = $.extend(d, options);
		if (d.idir.substring(d.idir.length - 1) !== "/") {
			d.idir += "/"
		}
		...
		var style = function() {
				if (d.css.indexOf("background:") === -1) {
					d.css += "body{background:white;}"
				}
				if (d.css.indexOf("background-image:") === -1) {
					d.css = d.css + "body{background-image:url(" + d.idir + ");background-position:top right;background-repeat:no-repeat;}"
				}
		...
		var toolbar = function() {
				var h = "";
				if (d.about && !in_array(d.toolbars[0], "about")) {
				...
				for (var k = 0; k < d.toolbars.length; k++) {
					...
					for (var i = 0; i < (toolbar.length); i++) {
						...
						if (undefined === toolbar[i]) {
							...
							h += "<td valign='middle' align='center'><select id='" + d.id + "_fontsize' onchange='global_hb[\"" + d.id + "\"].cmd(\"fontsize\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>- 字号 -</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option></select></td>"
						} else if (toolbar[i] === "fontfamily") {
							h += "<td valign='middle' align='center'><select id='" + d.id + "_fontfamily' onchange='global_hb[\"" + d.id + "\"].cmd(\"fontname\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>- 字体 -</option><option value='arial' style='font-family:arial;'>Arial</option><option value='宋体' style='font-family:宋体;'>宋体</option><option value='cursive' style='font-family:楷体;'>楷体</option><option value='georgia' style='font-family:georgia;'>Georgia</option><option value='monospace' style='font-family:monospace;'>Monospace</option><option value='tahoma' style='font-family:tahoma;'>Tahoma</option><option value='verdana' style='font-family:verdana;'>Verdana</option></select></td>"
						} else if (toolbar[i] === "formats") {
							h += "<td valign='middle' align='center'><select id='" + d.id + "_formats' onchange='global_hb[\"" + d.id + "\"].cmd(\"format\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>- 样式 -</option><option value='h1'>标题 1</option><option value='h2'>标题 2</option><option value='h3'>标题 3</option><option value='h4'>标题 4</option><option value='h5'>标题 5</option><option value='h6'>标题 6</option><option value='p'>段落</option><option value='pindent'>首行缩行</option></select></td>"
						} else if (toolbar[i] === "fontcolor") {
							h += "<td valign='middle' align='center'><select id='" + d.id + "_fontcolor' onchange='global_hb[\"" + d.id + "\"].cmd(\"fontcolor\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>-字体颜色-</option>";
							...
							h += "<td valign='middle' align='center'><select id='" + d.id + "_highlight' onchange='global_hb[\"" + d.id + "\"].cmd(\"backcolor\",this.options[this.selectedIndex].value)' style='font-size:12px;'><option value='' selected>-底纹-</option>";
							...
							h += "<td valign='middle' align='center'><select id='" + d.id + "_styles' onchange='global_hb[\"" + d.id + "\"].cmd(\"styles\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;' style='background:white;'><option value='' selected>-样式-</option>";
							...
							h += "<td valign='middle' align='center'><select id='" + d.id + "_syntax' onchange='global_hb[\"" + d.id + "\"].cmd(\"syntax\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' selected>-排版格式-</option>";
		...
		this._init = function(is_init) {
			...
			$(this).wrap("<table id='" + d.id + "_wrap' width='" + w + "' style='height:" + h + ";border:2px solid #E9EAEF;' cellspacing='0' cellpadding='0'><tr><td id='" + d.id + "_container'></td></tr></table>");
			...
			$("." + d.id + "_html_button").each(function() {
				...
				$(this).find("image").each(function() {
					$(this).width(d.tool_image_width).height(d.tool_image_height)
				});
				...
				$(this).mouseout(function() {
					$(this).css("border", "1px solid transparent").css("background", "transparent")
				})
			});
			...
			try {
				...
				$(iframe).css("width", "100%").css("height", 120).attr("id", d.id + "_html").css("border", "0");
		...
		this.cmd = function(cmd, arg1) {
			...
			if (cmd === "code") {
				...
				d.idoc.execCommand("createlink", false, prompt("粘贴网站地址:"))
			} else if (cmd === "image") {
				d.idoc.execCommand("insertimage", false, prompt("粘贴图片地址:"))
